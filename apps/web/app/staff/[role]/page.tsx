...
import AdminOrderOversight,{PaymentReconciliation} from './AdminOrderOversight';
...
export default function Dashboard({params}:{params:{role:string}}) {
  ...
  return <main className="dash">
    <aside className="sidebar"><div className="brand light"><span>IK</span> Indian Kitchen</div><nav><button className={tab==='orders'?'active':''} onClick={()=>setTab('orders')}><LayoutDashboard/> Live orders</button>{params.role==='admin'&&<><button className={tab==='branches'?'active':''} onClick={()=>setTab('branches')}><LayoutDashboard/> Branches</button><button className={tab==='menu'?'active':''} onClick={()=>openResource('menu')}><Menu/> Menu items</button><button className={tab==='categories'?'active':''} onClick={()=>openResource('categories')}><RefreshCcw/> Categories</button><button className={tab==='tables'?'active':''} onClick={()=>openResource('tables')}><QrCode/> Tables & QR</button><button className={tab==='staff'?'active':''} onClick={()=>openResource('staff')}><Users/> Staff</button><button className={tab==='analytics'?'active':''} onClick={()=>setTab('analytics')}><BarChart3/> Analytics</button><button className={tab==='reports'?'active':''} onClick={()=>setTab('reports')}><Download/> Reports</button><button className={tab==='reconciliation'?'active':''} onClick={()=>setTab('reconciliation')}><IndianRupee/> Reconciliation</button><button className={tab==='audit'?'active':''} onClick={()=>setTab('audit')}><RefreshCcw/> Audit log</button><button className={tab==='settings'?'active':''} onClick={()=>openResource('settings')}><Settings/> Restaurant settings</button></>}</nav><div className="staffMini"><span>{staff.name[0]}</span><div><b>{staff.name}</b><small>{staff.role}</small></div><button aria-label="Log out" onClick={()=>{localStorage.clear();router.push('/staff/login')}}><LogOut/></button></div></aside>
    <section className="workspace"><header><div><p className="eyebrow red">{staff.role} workspace</p><h1>{title}</h1></div>{params.role==='admin'?<select className="branchSwitcher" value={selectedBranch} onChange={event=>setSelectedBranch(event.target.value)}><option value="">All branches</option>{(resources.branches||[]).map(branch=><option value={branch.id} key={branch.id}>{branch.name}</option>)}</select>:<div className="live"><i/> Live</div>}</header>
      {loading&&<div className="adminLoading">Loading controlsâ€¦</div>}
      {tab==='orders'&&(params.role==='kitchen'?<KitchenBoard orders={visibleOrders} update={updateOrder}/>:params.role==='operator'?<OperatorWorkspace orders={visibleOrders} update={updateOrder} paid={markPaid} reload={load}/>:<AdminOrderOversight branchId={selectedBranch}/>)}
      {tab==='analytics'&&<AdvancedAnalytics branchId={selectedBranch}/>}
      {tab==='branches'&&<BranchManager branches={resources.branches||[]} reload={load}/>}
      {tab==='reports'&&<ReportCenter branchId={selectedBranch}/>}
      {tab==='reconciliation'&&<PaymentReconciliation branchId={selectedBranch}/>}
      {tab==='audit'&&<AuditLogView branchId={selectedBranch}/>}
...
async function regenerateQr(row:any,branchId:string,reload:()=>void){if(!confirm('Regenerate QR for '+row.name+'? The old QR will stop working.'))return;await api(`/api/admin/tables/${row.id}/regenerate-qr?branchId=${branchId}`,{method:'POST',body:'{}'});reload()}
